...
function openSimSidebar() {
  ...
  <style>
    ...
    .bar{padding:14px 16px;background:#e8edf2;border-bottom:1px solid #e2e8f0}
    ...
    select,input,textarea{width:100%;background:#f5f7fa;border:1px solid #cbd5e0;color:#2c3e50;border-radius:8px;padding:8px}
    .row{display:flex;gap:10px}
    .col{flex:1}
    button{background:#3b82f6;border:0;color:#fff;padding:10px 12px;border-radius:8px;cursor:pointer}
    button.sec{background:#e2e8f0}
    .pill{display:inline-block;background:#e2e8f0;padding:6px 8px;border-radius:999px;font-size:12px}
...
function buildATSRPanelDark_(row, parsed, keepSelections) {
  const makeEditable = (vals, name) => vals.map((v,i)=>`
    <div style="margin-bottom:6px;">
      ...
      <input type="text" id="${name}_text_${i}" value="${String(v).replace(/"/g,'&quot;')}" style="width:92%; font-size:13px; background:#f5f7fa; color:#2c3e50; border:1px solid #cbd5e0; border-radius:8px; padding:6px;">
    </div>
  `).join('');
...
function openImageSyncDefaults() {
  ...
  <style>
    ...
    .bar{padding:14px 16px;background:#e8edf2;border-bottom:1px solid #e2e8f0}
    button{background:#3b82f6;border:0;color:#fff;padding:8px 12px;border-radius:8px;cursor:pointer}
  </style>
  ...
  <div style="padding:12px;">
    ...
    <div style="margin-top:12px;">
      ...
      <button style="background:#e2e8f0" onclick="refresh()">Refresh Columns</button>
    </div>
  </div>
  <script>
    function save(){
...
function openMemoryTracker() {
  ...
  const html = HtmlService.createHtmlOutput(`
  <style>body{font-family:Arial;background:#f5f7fa;color:#2c3e50} button{background:#3b82f6;border:0;color:#fff;padding:8px 12px;border-radius:8px;cursor:pointer}</style>
  <div style="padding:16px;">
    <h3>${ICONS.puzzle} Memory Tracker</h3>
    ${list || '<p><em>No motifs stored.</em></p>'}
    <div style="margin-top:12px;">
      <button onclick="clearAll()">🧹 Clear All</button>
      <button style="background:#e2e8f0" onclick="markReusable()">♻️ Mark Selected as Reusable</button>
    </div>
  </div>
  <script>
...
function openSettingsPanel() {
  ...
  label{font-size:12px;color:#9aa3b2}
  input,select{width:100%;background:#f5f7fa;border:1px solid #cbd5e0;color:#2c3e50;border-radius:8px;padding:8px}
  button{background:#3b82f6;border:0;color:#fff;padding:8px 12px;border-radius:8px;cursor:pointer}
  ...
  <div class="card">
    ...
    <div style="margin-top:10px;">
      <button onclick="save()">Save</button>
      <button style="background:#e2e8f0" onclick="clearCache()">Clear Header Cache</button>
      <button style="background:#e2e8f0" onclick="pull()">Sync API from Settings sheet</button>
    </div>
  </div>
  <script>
    function save(){
