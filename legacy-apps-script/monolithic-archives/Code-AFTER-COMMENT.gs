...
function showFieldSelector() {
  ...
  try {
    ...
    html += '<button class="btn btn-primary" id="load-defaults-btn" onclick="loadDefaults()" style="background: #34a853;">🔄 Load Defaults</button>';
...
// DISABLED: This self-executing function was creating a duplicate menu
// that overwrote the main onOpen() menu. Waveform mapping features should
// be added to the main onOpen() function instead.
/*
...
function testApplyLogs() {
  PropertiesService.getDocumentProperties().deleteProperty('Sidebar_Logs');
  addAILog('🧪 TEST: Logs are working!');
...
function validateCategorization(categorizationData, masterSheet) {
  ...
  Logger.log('✅ Layer 5 passed: All checks complete');

  Logger.log('');
  Logger.log('✅ All 5 layers passed - safe to proceed');
  return true;
}
...
function findRowByLegacyCaseID(sheet, legacyCaseID) {
  addAILog('🔍 findRowByLegacyCaseID: Looking for "' + legacyCaseID + '"');
...
function findRowByCaseID(sheet, caseID) {
  addAILog('🔍 findRowByCaseID: Looking for "' + caseID + '"');
  addAILog('   Sheet: ' + (sheet ? sheet.getName() : 'NULL'));
...
function openAICategorization() {
  ...
  ui.showSidebar(HtmlService.createHtmlOutput(html).setTitle('🤖 AI Categorization').setWidth(400));
}
