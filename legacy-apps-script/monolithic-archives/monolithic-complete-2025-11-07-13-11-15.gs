...
function openSimSidebar() {
  ...
  Logger.log('📋 Step 4: Building HTML modal...');
...
/* DISABLED - Was overriding main menu
 ...
 */
function getStaticRecommendedFields(availableFields, selectedFields) {
  Logger.log('🔍 getStaticRecommendedFields() called with ' + availableFields.length + ' available, ' + selectedFields.length + ' selected');
...
function getFieldSelectorData() {
  ...
  try {
    ...
    var // Use exact Row 2 header field names (27 most important fields)
      // Use exact Row 2 header field names (only fields that actually exist)
      // Use exact Row 2 header field names (27 fields - all verified to exist)
      // Strategic 35-field selection optimized for AI pathway discovery
    ...
    addLog('   Step 4: Calling getRecommendedFields() (AI + fallback)');
    var recommendedFields = getRecommendedFields(availableFields, selectedFields);
 ...
 * Simply calls showFieldSelector() which loads the 35 defaults
 */
...
function showFieldSelector() {
  try {
    ...
    Logger.log('   Step 1: Building HTML content...');
    ...
    var htmlContent = '<!DOCTYPE html>' +
      ...
      '    var overlappingCount = selectedFields.filter(function(f) { return data.recommended.indexOf(f.name) !== -1; }).length;' +
