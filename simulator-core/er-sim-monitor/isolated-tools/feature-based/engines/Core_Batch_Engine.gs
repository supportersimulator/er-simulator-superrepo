/**
 * Core Batch Engine - Pure Business Logic
 *
 * Pure batch processing logic with NO UI dependencies.
 * Can be called from sidebar, API, CLI, or any other context.
 *
 * Functions:
 * - processOneInputRow_(row, settings) - Main batch processing
 * - validateVitalsFields_(vitals) - Input validation
 * - applyClinicalDefaults_(row) - Clinical defaults application
 * - tryParseJSON(str) - Safe JSON parsing
 *
 * No UI, no HTML, no button handlers - just pure logic.
 *
 * Generated: 2025-11-04T18:37:10.312Z
 */
