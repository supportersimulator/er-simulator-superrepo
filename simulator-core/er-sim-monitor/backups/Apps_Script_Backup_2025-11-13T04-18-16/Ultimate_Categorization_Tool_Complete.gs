...
function extractCasesForCategorization(data, headers) {
  // Use hardcoded column indices from proven working AI_Categorization_Backend.gs
  // This matches the original tool that successfully processed cases
  const cases = [];
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (caseID && caseID !== 'Case_Organization_Case_ID') {  // Skip header row if present
      cases.push({
        ...
        caseID: caseID,                     // Column A (idx 0)
        legacyCaseID: row[8] || '',         // Column I (idx 8): Case_Organization_Legacy_Case_ID
        ...
        chiefComplaint: row[4] || '',       // Column E (idx 4): Case_Organization_Case_Series_Order (approx)
        presentation: row[5] || '',         // Column F (idx 5): Case_Organization_Pathway_or_Course_Name (approx)
        diagnosis: row[6] || ''             // Column G (idx 6): Case_Organization_Difficulty_Level (approx)
      });
    }
  }
  return cases;
...
function buildCategorizationPrompt(cases, validSymptoms, validSystems) {
  ...
  prompt += '1. Base categorization on the MEDICAL DIAGNOSIS and CLINICAL PRESENTATION, not on metadata or case series names\n';
  prompt += '2. If a case mentions "nightmare" or other descriptive terms in its series name, ignore those - focus ONLY on the actual medical condition\n';
  prompt += '3. Categorize by PRIMARY presenting symptom and PRIMARY affected body system\n';
  prompt += '4. Psychiatric (PSY) should ONLY be used if the diagnosis itself is psychiatric (e.g., acute psychosis, suicidal ideation)\n';
  prompt += '5. Ignore case difficulty labels, teaching series names, or educational metadata - focus on the medical facts\n\n';
  prompt += 'INSTRUCTIONS:\n';
  prompt += '1. For each case, select ONE symptom code from the valid list above based on chief complaint/presentation\n';
  prompt += '2. For each case, select ONE system category from the valid list above based on diagnosis\n';
  prompt += '3. Provide the full name/description for the symptom\n';
  prompt += '4. Provide brief reasoning that references the actual diagnosis, not case metadata\n\n';
  prompt += 'CASES TO CATEGORIZE:\n';
  cases.forEach(function(c, i) {
    prompt += (i + 1) + '. Case ID: ' + c.caseID + '\n';
    prompt += '   Chief Complaint: ' + c.chiefComplaint + '\n';
    prompt += '   Presentation: ' + c.presentation + '\n';
    prompt += '   Diagnosis: ' + c.diagnosis + '\n\n';
  });
  prompt += 'CRITICAL: You MUST return a valid JSON array with EXACTLY these fields for each case:\n';
  prompt += '[\n';
  prompt += '  {\n';
  prompt += '    "caseID": "exact case ID from above",\n';
  ...
  prompt += '  }\n';
  prompt += ']\n\n';
  prompt += 'Return ONLY the JSON array, no other text. ALL fields are required for each case.\n';
  return prompt;
}
...
function writeCategorizationResults(cases, categorizations) {
  ...
  // FIX: Get mapping for fallback symptom names
  const mapping = getAccronymMapping();
  ...
  cases.forEach(function(caseData, idx) {
    ...
    const suggestedSymptomName = cat.symptomName || mapping[cat.symptom] || '';  // FIX: Fallback to mapping
...
function applyUltimateCategorizationToMaster() {
  ...
  try {
    ...
    if (resultsLastRow <= 1) return { success: false, error: 'No results to apply' };

    // Read results with Final columns (15 columns: A-O)
    const resultsData = resultsSheet.getRange(2, 1, resultsLastRow - 1, 15).getValues();
    // FIXED: Use GID instead of name to ensure correct sheet
    const masterSheet = getMasterScenarioConvertSheet_();
    if (!masterSheet) return { success: false, error: 'Master sheet not found (GID: ' + MASTER_SCENARIO_CONVERT_GID + ')' };

    // Use hardcoded column indices matching verified structure
    const caseIDIdx = 0;       // Column A: Case_Organization_Case_ID
    ...
    resultsData.forEach(function(resultRow) {
      ...
      if (masterRowNum && finalSymptom && finalSystem) {
        ...
        updatedCount++;
        addUltimateCategorizationLog('   Updated: ' + caseID + ' → ' + finalSymptom + ' / ' + finalSystem);
      }
    });
