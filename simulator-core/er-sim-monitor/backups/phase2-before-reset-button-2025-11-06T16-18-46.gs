 ...
 */
var TEST_SPREADSHEET_ID = '1t3qN8e537ghl38GTsXbVG8ML8OZtK2XyUpDiMQjnGAI';
...
function performHolisticAnalysis_() {
  ...
  // Define complete field map with ALL 27 fields for AI pathway discovery
  const fieldMap = {
    // BASIC INFO (3 fields)
  ...
  // Resolve ALL column indices using header cache
  const indices = resolveColumnIndices_(fieldMap);

  // Defensive: Check if critical columns were found
  if (indices.caseId === -1 || indices.sparkTitle === -1) {
    throw new Error('Could not find required columns. CaseID idx: ' + indices.caseId + ', Spark idx: ' + indices.sparkTitle);
  }

  // Collect all cases with FULL metadata (27 fields)
  const allCases = [];
  const systemDistribution = {};
  const pathwayDistribution = {};
  let unassignedCount = 0;

  // Process in 25-row batches to avoid timeout
  const BATCH_SIZE = 25;
  ...
  // Identify high-value pathway opportunities (KEEP EXISTING LOGIC)
  const pathwayOpportunities = identifyPathwayOpportunities_(allCases, systemDistribution);

  // Generate insights (KEEP EXISTING LOGIC)
