...
async function deployViaAPI() {
  try {
    ...
    // Function 1: extractCasesForCategorization
    const newExtractFunction = `function extractCasesForCategorization(data, headers) {
  const cases = [];
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (caseID && caseID !== 'Case_Organization_Case_ID') {
      cases.push({
        ...
        revealTitle: row[2] || '',
        legacyCaseID: row[8] || '',
        currentSymptomCode: row[15] || '',
        currentSystemCode: row[16] || '',
        currentSymptomName: row[17] || '',
        currentSystemName: row[18] || ''
      });
    }
  }
  return cases;
    ...
    // Function 2: buildCategorizationPrompt
    const newPromptFunction = `function buildCategorizationPrompt(cases, validSymptoms, validSystems) {
  ...
  return prompt;
    ...
    // Function 3: writeCategorizationResults
    const newWriteFunction = `function writeCategorizationResults(cases, categorizations) {
  ...
  if (!resultsSheet) {
    ...
    resultsSheet.getRange(1, 1, 1, 16).setValues([['Case_Organization_Case_ID', 'Legacy_Case_ID', 'Row_Index', 'Case_Organization_Spark_Title', 'Case_Organization_Reveal_Title', 'Suggested_Symptom_Code', 'Suggested_Symptom_Name', 'Suggested_System_Code', 'Suggested_System_Name', 'AI_Reasoning', 'Status', 'User_Decision', 'Final_Symptom_Code', 'Final_System_Code', 'Final_Symptom_Name', 'Final_System_Name']]);
  ...
  addUltimateCategorizationLog('         Final row: ' + (nextRow - 1));
    ...
    // Function 4: applyUltimateCategorizationToMaster
    const newApplyFunction = `function applyUltimateCategorizationToMaster() {
  ...
  addUltimateCategorizationLog('✅ Found all required columns');
  ...
  for (let i = 1; i < resultsData.length; i++) {
    ...
    for (let j = 2; j < masterData.length; j++) {
      if (masterData[j][0] === caseID) {
        ...
        updatedCount++;
        if (updatedCount <= 3) {
          addUltimateCategorizationLog('  ✅ Row ' + (j+1) + ' - ' + caseID + ': P=' + finalSymptomCode + ', Q=' + finalSystemCode + ', R=' + finalSymptomName + ', S=' + finalSystemName);
        }
        break;
      }
    }
  ...
  addUltimateCategorizationLog('✅ Apply to Master Complete: ' + updatedCount + ' cases updated (P,Q,R,S)');
}`;

    // Replace each function
    const replacements = [
      { name: 'extractCasesForCategorization', newCode: newExtractFunction },
      { name: 'buildCategorizationPrompt', newCode: newPromptFunction },
      { name: 'writeCategorizationResults', newCode: newWriteFunction },
      { name: 'applyUltimateCategorizationToMaster', newCode: newApplyFunction }
    ];

    let successCount = 0;

    replacements.forEach(({ name, newCode }) => {
      // Find the function start
      const funcStart = code.indexOf(`function ${name}(`);
      if (funcStart === -1) {
      ...
      if (funcEnd === -1) {
        console.log(`⚠️  Could not find end of ${name}()`);
        return;
      }

      // Replace the function
      code = code.substring(0, funcStart) + newCode + code.substring(funcEnd);
      console.log(`✅ Replaced ${name}()`);
      successCount++;
    });

    if (successCount === 0) {
      console.log('\n❌ No functions were replaced. Aborting deployment.');
      return;
    }

    console.log(`\n📊 Successfully updated ${successCount}/4 functions`);
    console.log(`📏 Code length: ${originalLength} → ${code.length} (${code.length > originalLength ? '+' : ''}${code.length - originalLength})`);

    targetFile.source = code;
    ...
    console.log('═══════════════════════════════════════════════════════════════');
    console.log('🎉 CATEGORIZATION FIX DEPLOYED!');
    console.log('═══════════════════════════════════════════════════════════════\n');
    console.log(`Functions updated: ${successCount}/4`);
    console.log('\nNext steps:');
    ...
    console.log('5. Check debug logs for first 3 cases');
    console.log('6. Verify columns have correct data\n');
