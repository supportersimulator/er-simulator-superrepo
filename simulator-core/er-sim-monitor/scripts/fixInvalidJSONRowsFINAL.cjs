...
async function fixInvalidRows() {
  console.log('\n🔧 FIXING 2 ROWS WITH INVALID JSON (N/A values)\n');
  ...
  function standardize(obj) {
    ...
    if (std.bp === null) {
      std.bp = {sys: null, dia: null};
    }

    std.waveform = std.waveform || 'asystole_ecg'; // N/A vitals = asystole
  ...
  const updates = [
    {
      range: `${SHEET_NAME}!${vitalsColumn}53`,
      values: [[JSON.stringify(std53)]]
    },
    {
      range: `${SHEET_NAME}!${vitalsColumn}137`,
