...
/**
 * RESTORE PROJECT FROM BACKUP WITH MANIFEST
 * Fetches current manifest, then restores backup files
 */
...
console.log('\n🔄 RESTORING PROJECT FROM BACKUP (WITH MANIFEST)\n');
console.log('═══════════════════════════════════════════════════════════════\n');
...
async function restoreProject() {
  try {
    ...
    console.log('📥 Fetching current project to get manifest...\n');
    ...
    if (!manifestFile) {
      throw new Error('Could not find appsscript manifest in current project');
    }
    ...
    filesToRestore.push(manifestFile);
    ...
    filesToRestore.forEach((file, i) => {
      if (file.name === 'appsscript') {
        console.log(`   ${i + 1}. ${file.name} (${file.type}) - MANIFEST (kept current)`);
      } else {
        console.log(`   ${i + 1}. ${file.name} (${file.type}) - RESTORED FROM BACKUP`);
      }
    });
    console.log('\n═══════════════════════════════════════════════════════════════\n');
    console.log('🎯 PROJECT RESTORED TO CLEAN STATE\n');
    console.log('All code files restored from backup made before the failed modification.\n');
    console.log('Manifest file preserved from current project.\n');
