/**
 * ═══════════════════════════════════════════════════════════════════════════
 * ULTIMATE CATEGORIZATION TOOL - PHASE 2E
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * Production-grade AI categorization system with Browse tabs
 *
 * Features:
 * - Tab navigation (Categorize, Browse Symptom, Browse System, Settings)
 * - Phase 2A-2D: AI Categorization, Apply, Export, Clear (COMPLETE)
 * - Phase 2E: Browse by Symptom/System with visual category lists (NEW)
 * - Phase 2F-2G: Coming soon (Settings, AI Suggestions)
 *
 * Created: 2025-11-11
 * Version: 2E.0.0
 * ═══════════════════════════════════════════════════════════════════════════
...
/**
 * Get CSS styles (Phase 2E: Added tab navigation styles)
 */
...
/**
 * Get HTML body content (Phase 2E: Added tab structure)
 */
function getUltimateCategorizationBody() {
  ...
  // TAB 1: CATEGORIZE (Phase 2A-2D - existing)
  ...
  // TAB 2: BROWSE BY SYMPTOM (Phase 2E - NEW)
  ...
  body += '      <div id="symptomCategoryList">';
  body += '        <div class="empty-state">';
  body += '          <div class="empty-state-icon">🔍</div>';
  body += '          <div class="empty-state-text">Loading categories...</div>';
  body += '        </div>';
  body += '      </div>';
  body += '    </div>';
  body += '    <div class="case-list">';
  body += '      <div class="panel-title">Cases in Category</div>';
  body += '      <div id="symptomCaseList">';
  body += '        <div class="empty-state">';
  body += '          <div class="empty-state-icon">📋</div>';
  body += '          <div class="empty-state-text">Select a category to view cases</div>';
  body += '        </div>';
  body += '      </div>';
  body += '    </div>';
  body += '  </div>';
  ...
  // TAB 3: BROWSE BY SYSTEM (Phase 2E - NEW)
  body += '<div id="tabBrowseSystem" class="tab-content">';
  body += '  <div class="browse-container">';
  body += '    <div class="category-list">';
  body += '      <div class="panel-title">System Categories</div>';
  body += '      <div id="systemCategoryList">';
  body += '        <div class="empty-state">';
  body += '          <div class="empty-state-icon">🏥</div>';
  body += '          <div class="empty-state-text">Loading systems...</div>';
  body += '        </div>';
  body += '      </div>';
  body += '    </div>';
  body += '    <div class="case-list">';
  body += '      <div class="panel-title">Cases in System</div>';
  body += '      <div id="systemCaseList">';
  body += '        <div class="empty-state">';
  body += '          <div class="empty-state-icon">📋</div>';
  body += '          <div class="empty-state-text">Select a system to view cases</div>';
  body += '        </div>';
  body += '      </div>';
  body += '    </div>';
  body += '  </div>';
  ...
  // TAB 4: SETTINGS (Phase 2F - PLACEHOLDER)
  body += '<div id="tabSettings" class="tab-content">';
  body += '  <div style="padding:40px;text-align:center;">';
  body += '    <div style="font-size:48px;margin-bottom:20px;">⚙️</div>';
  body += '    <div style="font-size:18px;color:#8b92a0;margin-bottom:12px;">Settings Tab</div>';
  body += '    <div style="font-size:14px;color:#666;">Coming in Phase 2F: Category Management & AI Suggestions</div>';
  body += '  </div>';
  body += '</div>'; // End Tab 4
...
/**
 * Get JavaScript code (Phase 2E: Added tab switching and browse functions)
 */
function getUltimateCategorizationJavaScript() {
  let js = '';

  // PHASE 2E: Tab switching
  js += 'function switchTab(tabName) {\n';
  js += '  // Hide all tabs\n';
  js += '  var tabs = document.querySelectorAll(".tab-content");\n';
  js += '  tabs.forEach(function(tab) {\n';
  js += '    tab.classList.remove("active");\n';
  js += '  });\n';
  js += '  \n';
  js += '  // Remove active from all tab buttons\n';
  js += '  var buttons = document.querySelectorAll(".tab-button");\n';
  js += '  buttons.forEach(function(btn) {\n';
  js += '    btn.classList.remove("active");\n';
  js += '  });\n';
  js += '  \n';
  js += '  // Show selected tab\n';
  js += '  var targetTab = null;\n';
  js += '  var targetButton = null;\n';
  js += '  \n';
  js += '  if (tabName === "categorize") {\n';
  js += '    targetTab = document.getElementById("tabCategorize");\n';
  js += '    targetButton = document.querySelectorAll(".tab-button")[0];\n';
  js += '  } else if (tabName === "browseSymptom") {\n';
  js += '    targetTab = document.getElementById("tabBrowseSymptom");\n';
  js += '    targetButton = document.querySelectorAll(".tab-button")[1];\n';
  js += '    loadSymptomCategories();\n';
  js += '  } else if (tabName === "browseSystem") {\n';
  js += '    targetTab = document.getElementById("tabBrowseSystem");\n';
  js += '    targetButton = document.querySelectorAll(".tab-button")[2];\n';
  js += '    loadSystemCategories();\n';
  js += '  } else if (tabName === "settings") {\n';
  js += '    targetTab = document.getElementById("tabSettings");\n';
  js += '    targetButton = document.querySelectorAll(".tab-button")[3];\n';
  js += '  }\n';
  js += '  \n';
  js += '  if (targetTab) targetTab.classList.add("active");\n';
  js += '  if (targetButton) targetButton.classList.add("active");\n';
  js += '}\n';
  js += '\n';

  // PHASE 2E: Load symptom categories
  js += 'function loadSymptomCategories() {\n';
  js += '  var container = document.getElementById("symptomCategoryList");\n';
  js += '  container.innerHTML = "<div class=\\"empty-state\\"><div class=\\"empty-state-icon\\">⏳</div><div class=\\"empty-state-text\\">Loading categories...</div></div>";\n';
  js += '  \n';
  js += '  google.script.run\n';
  js += '    .withSuccessHandler(function(stats) {\n';
  js += '      if (!stats || !stats.symptoms) {\n';
  js += '        container.innerHTML = "<div class=\\"empty-state\\"><div class=\\"empty-state-icon\\">❌</div><div class=\\"empty-state-text\\">No categories found</div></div>";\n';
  js += '        return;\n';
  js += '      }\n';
  js += '      \n';
  js += '      var html = "";\n';
  js += '      var symptoms = stats.symptoms;\n';
  js += '      var keys = Object.keys(symptoms).sort();\n';
  js += '      \n';
  js += '      keys.forEach(function(symptomCode) {\n';
  js += '        var data = symptoms[symptomCode];\n';
  js += '        html += "<div class=\\"category-item\\" onclick=\\"loadSymptomCases(\'" + symptomCode + "\')\\">";\n';
  js += '        html += "  <div class=\\"category-name\\">" + symptomCode + " - " + (data.name || "Unknown") + "</div>";\n';
  js += '        html += "  <div class=\\"category-count\\">" + data.total + " cases</div>";\n';
  js += '        html += "</div>";\n';
  js += '      });\n';
  js += '      \n';
  ...
  js += '      container.innerHTML = "<div class=\\"empty-state\\"><div class=\\"empty-state-icon\\">❌</div><div class=\\"empty-state-text\\">Error loading categories</div></div>";\n';
  js += '      console.error(error);\n';
  js += '    })\n';
  js += '    .getCategoryStatistics();\n';
  js += '}\n';
  js += '\n';

  // PHASE 2E: Load system categories
  js += 'function loadSystemCategories() {\n';
  js += '  var container = document.getElementById("systemCategoryList");\n';
  js += '  container.innerHTML = "<div class=\\"empty-state\\"><div class=\\"empty-state-icon\\">⏳</div><div class=\\"empty-state-text\\">Loading systems...</div></div>";\n';
  js += '  \n';
  js += '  google.script.run\n';
  js += '    .withSuccessHandler(function(stats) {\n';
  js += '      if (!stats || !stats.systems) {\n';
  js += '        container.innerHTML = "<div class=\\"empty-state\\"><div class=\\"empty-state-icon\\">❌</div><div class=\\"empty-state-text\\">No systems found</div></div>";\n';
  js += '        return;\n';
  js += '      }\n';
  js += '      \n';
  js += '      var html = "";\n';
  js += '      var systems = stats.systems;\n';
  js += '      var keys = Object.keys(systems).sort();\n';
  js += '      \n';
  js += '      keys.forEach(function(systemName) {\n';
  js += '        var data = systems[systemName];\n';
  js += '        html += "<div class=\\"category-item\\" onclick=\\"loadSystemCases(\'" + systemName + "\')\\">";\n';
  js += '        html += "  <div class=\\"category-name\\">" + systemName + "</div>";\n';
  js += '        html += "  <div class=\\"category-count\\">" + data.total + " cases</div>";\n';
  js += '        html += "</div>";\n';
  js += '      });\n';
  js += '      \n';
  ...
  js += '    })\n';
  js += '    .withFailureHandler(function(error) {\n';
  js += '      container.innerHTML = "<div class=\\"empty-state\\"><div class=\\"empty-state-icon\\">❌</div><div class=\\"empty-state-text\\">Error loading systems</div></div>";\n';
  js += '      console.error(error);\n';
  js += '    })\n';
  js += '    .getCategoryStatistics();\n';
  js += '}\n';
  ...
  // PHASE 2E: Load cases for symptom
  js += 'function loadSymptomCases(symptomCode) {\n';
  js += '  var container = document.getElementById("symptomCaseList");\n';
  js += '  container.innerHTML = "<div class=\\"empty-state\\"><div class=\\"empty-state-icon\\">⏳</div><div class=\\"empty-state-text\\">Loading cases...</div></div>";\n';
  js += '  \n';
  js += '  google.script.run\n';
  js += '    .withSuccessHandler(function(cases) {\n';
  js += '      if (!cases || cases.length === 0) {\n';
  js += '        container.innerHTML = "<div class=\\"empty-state\\"><div class=\\"empty-state-icon\\">📭</div><div class=\\"empty-state-text\\">No cases found</div></div>";\n';
  js += '        return;\n';
  js += '      }\n';
  js += '      \n';
  js += '      var html = "";\n';
  js += '      cases.forEach(function(c) {\n';
  js += '        var statusClass = "status-" + c.status;\n';
  js += '        var statusIcon = c.status === "match" ? "✅" : (c.status === "conflict" ? "⚠️" : "🆕");\n';
  js += '        \n';
  js += '        html += "<div class=\\"case-card\\">";\n';
  js += '        html += "  <div class=\\"case-header\\">";\n';
  js += '        html += "    <div class=\\"case-id\\">" + c.caseID + "</div>";\n';
  js += '        html += "    <div class=\\"case-status " + statusClass + "\\">" + statusIcon + " " + c.status + "</div>";\n';
  js += '        html += "  </div>";\n';
  js += '        html += "  <div class=\\"case-detail\\">";\n';
  js += '        html += "    <strong>Symptom:</strong> " + (c.currentSymptom || "none") + " → " + c.finalSymptom + "<br>";\n';
  js += '        html += "    <strong>System:</strong> " + c.finalSystem;\n';
  js += '        if (c.reasoning) {\n';
  js += '          html += "<br><strong>Reasoning:</strong> " + c.reasoning;\n';
  js += '        }\n';
  js += '        html += "  </div>";\n';
  js += '        html += "</div>";\n';
  js += '      });\n';
  ...
  js += '    })\n';
  js += '    .withFailureHandler(function(error) {\n';
  js += '      container.innerHTML = "<div class=\\"empty-state\\"><div class=\\"empty-state-icon\\">❌</div><div class=\\"empty-state-text\\">Error loading cases</div></div>";\n';
  js += '      console.error(error);\n';
  js += '    })\n';
  js += '    .getCasesForCategory("symptom", symptomCode);\n';
  js += '}\n';
  js += '\n';

  // PHASE 2E: Load cases for system
  js += 'function loadSystemCases(systemName) {\n';
  ...
  js += '        html += "  </div>";\n';
  js += '        html += "  <div class=\\"case-detail\\">";\n';
  js += '        html += "    <strong>Symptom:</strong> " + c.finalSymptom + "<br>";\n';
  js += '        html += "    <strong>System:</strong> " + (c.currentSystem || "none") + " → " + c.finalSystem;\n';
  js += '        if (c.reasoning) {\n';
  js += '          html += "<br><strong>Reasoning:</strong> " + c.reasoning;\n';
  js += '        }\n';
  js += '        html += "  </div>";\n';
  ...
  js += '      console.error(error);\n';
  js += '    })\n';
  js += '    .getCasesForCategory("system", systemName);\n';
  js += '}\n';
  js += '\n';
...
// ═══════════════════════════════════════════════════════════════════════════
// PHASE 2E: BROWSE BY SYMPTOM/SYSTEM - BACKEND FUNCTIONS
// ═══════════════════════════════════════════════════════════════════════════

/**
 * Get category statistics (counts per symptom/system)
 */
function getCategoryStatistics() {
  ...
  try {
    ...
    if (!resultsSheet) {
      addUltimateCategorizationLog('⚠️  No AI_Categorization_Results sheet found');
      return { symptoms: {}, systems: {} };
    }

    const lastRow = resultsSheet.getLastRow();
    if (lastRow <= 2) {
      addUltimateCategorizationLog('⚠️  No results data found');
      return { symptoms: {}, systems: {} };
    }

    addUltimateCategorizationLog('📊 Reading results data (rows 3-' + lastRow + ')...');
    ...
    data.forEach(function(row) {
      ...
      if (finalSymptom) {
        if (!symptomStats[finalSymptom]) {
          symptomStats[finalSymptom] = {
            name: row[6] || finalSymptom, // Suggested_Symptom_Name
            total: 0,
            match: 0,
            conflict: 0,
            new: 0
    ...
    addUltimateCategorizationLog('   ✅ Symptoms: ' + Object.keys(symptomStats).length);
    addUltimateCategorizationLog('   ✅ Systems: ' + Object.keys(systemStats).length);
    addUltimateCategorizationLog('');
...
function getCasesForCategory(type, value) {
  addUltimateCategorizationLog('═══════════════════════════════════════════════════════════');
  addUltimateCategorizationLog('🔍 GET CASES FOR CATEGORY');
  addUltimateCategorizationLog('═══════════════════════════════════════════════════════════');
  addUltimateCategorizationLog('   Type: ' + type);
  addUltimateCategorizationLog('   Value: ' + value);
  addUltimateCategorizationLog('');
  ...
  try {
    ...
    if (!resultsSheet) {
      addUltimateCategorizationLog('⚠️  No results sheet found');
      return [];
    }
    ...
    addUltimateCategorizationLog('📊 Reading results data...');
    const data = resultsSheet.getRange(3, 1, lastRow - 2, 14).getValues();

    // Filter cases
    const cases = [];
    const columnIndex = type === 'symptom' ? 12 : 13; // M=12 (Final_Symptom), N=13 (Final_System)
    ...
    data.forEach(function(row) {
      ...
      if (caseID && categoryValue === value) {
        cases.push({
          caseID: caseID,
          currentSymptom: row[2],      // Current_Symptom
          currentSystem: row[3],        // Current_System
          finalSymptom: row[12],        // Final_Symptom
          finalSystem: row[13],         // Final_System
          reasoning: row[8],            // Reasoning
