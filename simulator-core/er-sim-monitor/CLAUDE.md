...
# Standing Protocol: Atlas → Hermes Shared Ledger Final-Report Sync

Atlas MUST permanently follow this protocol:

1. Atlas MUST write ONLY its **final, end-of-session report** to:
   google-drive-code/shared/atlas-hermes-ledger.md

   “End-of-session report” means:
   - The final consolidated summary Atlas writes *after completing all tasks*
   - The large, structured, full-state report Atlas produces at the end of a workflow
   - NOT intermediate updates, NOT partial results, NOT step-by-step output

2. The entry MUST be an exact, word-for-word copy of the final report shown to the user:
   - No compression
   - No summarizing
   - No reformatting

3. The appended entry MUST follow this format:

   ### [ATLAS REPORT — {{timestamp}}]
   <full final report exactly as shown to the user>
   ### END ATLAS REPORT

4. Atlas MUST append (add to end) — never overwrite — so the ledger forms a chronological history.

5. Atlas MUST ALWAYS write the final report to the ledger **before** sending that same final report to the user.

6. If the ledger does not exist or is missing/corrupted, Atlas MUST:
   - recreate the file immediately
   - restore the header:
     "# Atlas ↔ Hermes Shared Ledger\n\n(Initialized — ready for reports)\n"
   - then write the final report

7. This protocol is PERMANENT and MUST persist across:
   - session resets
   - window reloads
   - VS Code restarts
   - model drift
   - Atlas reinitialization

## HEARTBEAT PROTOCOL (ATLAS)
Before performing any workflow, Atlas must check the ledger:
google-drive-code/shared/atlas-hermes-ledger.md.

If the most recent entry is:
- an ATLAS REPORT → do nothing.
- a HERMES REVIEW → do nothing.
- an ATLAS HEARTBEAT less than 2 hours old → do nothing.
- a HERMES HEARTBEAT less than 2 hours old → do nothing.
- otherwise → WRITE A HEARTBEAT:

### [ATLAS HEARTBEAT — {timestamp}]
No work completed this session. State synchronized.
### END ATLAS HEARTBEAT
