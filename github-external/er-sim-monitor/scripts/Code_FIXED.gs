...
function openSimSidebar() {
  ...
  <style>
    body{font-family:Arial;margin:0;background:#0f1115;color:#e7eaf0}
    .bar{padding:14px 16px;background:#1b1f2a;border-bottom:1px solid #2a3040}
    h2{margin:0;font-size:16px;letter-spacing:.3px}
    .wrap{padding:16px}
    .card{background:#141824;border:1px solid #2a3040;border-radius:10px;padding:14px;margin-bottom:12px}
    label{font-size:12px;color:#9aa3b2}
    select,input,textarea{width:100%;background:#0f1115;border:1px solid #30384b;color:#e7eaf0;border-radius:8px;padding:8px}
    ...
    .pill{display:inline-block;background:#2a3040;padding:6px 8px;border-radius:999px;font-size:12px}
...
function runATSRTitleGenerator(continueRow, keepSelections) {
  ...
  // Ask AI
  const prompt = `
You are creating Spark/Reveal titles & a concise review for a sim case.

Rules:
- Spark: "<Symptom> (<Age Sex>): <Spark Phrase>"  (e.g., "Chest Pain (47 M): Dizzy and Sweaty"), no diagnosis.
- Reveal: "<Diagnosis> (<Age Sex>): <Learning Objective>"
- Provide 10 Case IDs. Use system/symptom prefixes: SOB, CP, ABD, GI, TR, NE, OB, GU, MSK, CV, PULM, ID, RE, GYN, PEDGI, PEDCV, etc.
  - Pediatric: 8 chars (e.g., PEDGI012); others: 7 chars (e.g., GI01234) — uppercase letters + digits only.
- Provide a structured Case Summary (additions to existing summary rather than replacement):
   Patient_Summary (concise, 1–2 sentences),
   Key_Intervention (short phrase),
   Core_Takeaway (short phrase),
   Defining_Characteristic_Options (5 unique, humanizing; avoid motifs already used).

Avoid motifs already seen: ${usedMemory}
Avoid duplication with existing text (sample):
...
Output JSON:
{
  ...
  "Case_Summary": {
    ...
    "Key_Intervention": "...",
    "Core_Takeaway": "...",
    "Defining_Characteristic_Options": ["x5"]
  }
}
`;

  const ai = callOpenAI(prompt, 0.6);
...
function buildATSRPanelDark_(row, parsed, keepSelections) {
  const makeEditable = (vals, name) => vals.map((v,i)=>`
    <div style="margin-bottom:6px;">
      ...
      <input type="text" id="${name}_text_${i}" value="${String(v).replace(/"/g,'&quot;')}" style="width:92%; font-size:13px; background:#0f1115; color:#e7eaf0; border:1px solid #30384b; border-radius:8px; padding:6px;">
    </div>
  `).join('');
  ...
  <script>
    ...
    function apply(next){
      ...
      google.script.run
        ...
        .applyATSRSelectionsWithDefiningAndMemory(${row}, s.spark, s.reveal, s.caseID, s.define);
    }
    function keepRegen(){
      const s=getSel();
      google.script.run
...
function openImageSyncDefaults() {
  ...
  const html = HtmlService.createHtmlOutput(`
  <style>
    body{font-family:Arial;background:#0f1115;color:#e7eaf0}
    table{width:100%;border-collapse:collapse}
    td,th{border:1px solid #2a3040;padding:8px}
    .bar{padding:14px 16px;background:#1b1f2a;border-bottom:1px solid #2a3040}
  ...
  <div style="padding:12px;">
    ...
    <div style="margin-top:12px;">
      ...
      <button style="background:#2a3040" onclick="refresh()">Refresh Columns</button>
    </div>
  </div>
  <script>
    function save(){
...
function openMemoryTracker() {
  ...
  const html = HtmlService.createHtmlOutput(`
  <style>body{font-family:Arial;background:#0f1115;color:#e7eaf0} button{background:#2357ff;border:0;color:#fff;padding:8px 12px;border-radius:8px;cursor:pointer}</style>
  <div style="padding:16px;">
    <h3>${ICONS.puzzle} Memory Tracker</h3>
    ${list || '<p><em>No motifs stored.</em></p>'}
    <div style="margin-top:12px;">
      <button onclick="clearAll()">🧹 Clear All</button>
      <button style="background:#2a3040" onclick="markReusable()">♻️ Mark Selected as Reusable</button>
    </div>
  </div>
  <script>
...
function openSettingsPanel() {
  ...
  const html = HtmlService.createHtmlOutput(`
  <style>body{font-family:Arial;background:#0f1115;color:#e7eaf0}
  label{font-size:12px;color:#9aa3b2}
  input,select{width:100%;background:#0f1115;border:1px solid #30384b;color:#e7eaf0;border-radius:8px;padding:8px}
  button{background:#2357ff;border:0;color:#fff;padding:8px 12px;border-radius:8px;cursor:pointer}
  .card{background:#141824;border:1px solid #2a3040;border-radius:10px;padding:14px;margin:12px}
  </style>
  <div class="card">
    ...
    <div style="margin-top:10px;">
      <button onclick="save()">Save</button>
      <button style="background:#2a3040" onclick="clearCache()">Clear Header Cache</button>
      <button style="background:#2a3040" onclick="pull()">Sync API from Settings sheet</button>
    </div>
  </div>
  <script>
    function save(){
