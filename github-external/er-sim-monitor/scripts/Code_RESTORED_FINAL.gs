...
function buildATSRPanelDark_(row, parsed, keepSelections) {
  const makeEditable = (vals, name) => vals.map((v,i)=>`
    <div style="margin-bottom:6px;">
      ...
      <input type="text" id="${name}_text_${i}" value="${String(v).replace(/"/g,'&quot;')}" style="width:92%; font-size:13px; background:#f5f7fa; color:#2c3e50; border:1px solid #30384b; border-radius:8px; padding:6px;">
    </div>
  `).join('');
