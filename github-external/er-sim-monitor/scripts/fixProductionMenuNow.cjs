...
/**
 * FIX PRODUCTION MENU - ADD ONOPEN FUNCTION
 */
...
async function fix() {
  try {
    ...
    // Remove any existing onOpen functions
    console.log('🗑️  Removing old onOpen functions...\n');

    // More aggressive removal - remove ALL onOpen variations
    code = code.replace(/function onOpen\([^)]*\)\s*\{[^}]*\}/g, '');
    code = code.replace(/function onOpen\([^)]*\)\s*\{[\s\S]*?^\}/gm, '');

    console.log('✅ Removed old onOpen functions\n');

    // Add new onOpen function at the beginning
    console.log('📝 Adding new onOpen function...\n');
...
`;
    ...
    console.log('✅ Added new onOpen function\n');
    ...
    console.log('   1. Refresh your production spreadsheet (Cmd+Shift+R)\n');
