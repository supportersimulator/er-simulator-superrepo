...
function showFieldSelector() {
  ...
  const html =
    ...
    '  <div style="display: flex; gap: 10px;"><button class="btn-continue" onclick="continueToCache()">Continue to Cache →</button></div>' +
    '</div>' +
    '<script>' +
    'let allFields = [];' +
    'let selectedFields = [];' +
  ...
  Logger.log('📋 Step 6: Creating HTML output...');
