...
function showFieldSelector() {
  try {
    ...
    const html =
      ...
      '.log-panel { position: fixed; bottom: 0; left: 0; right: 0; background: #1e1e1e; color: #d4d4d4; padding: 15px; max-height: 200px; overflow-y: auto; border-top: 3px solid #007acc; }' +
      '.log-panel pre { margin: 0; font-family: "Courier New", monospace; font-size: 12px; white-space: pre-wrap; }' +
      ...
      '<body>' +
      '<div class="content">' +
      '<h2>Field Selector (Test Version)</h2>' +
      ...
      '<span class="log-title">📋 Live Execution Log</span>' +
      '<button class="btn-copy" onclick="copyLogs()">📋 Copy Logs</button>' +
      '</div>' +
      '<pre id="log"></pre>' +
      '</div>' +
      '<script>' +
      ...
      'log("📋 Rendering " + Math.min(50, fields.length) + " fields...");' +
      ...
      'log("✅ Field selector loaded successfully!");' +
